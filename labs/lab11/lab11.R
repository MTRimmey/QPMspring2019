...
fake.tab <- matrix(c(762,327,468,484, 239, 677), nrow=3, byrow=TRUE)
dimnames(fake.tab) <- list(Party=c("Democrat", "Independent", "Republican"),
...
mosaicplot( t(cancerTable), col = c("blue"))
...
STANDARD ERROR
seLn0R <- as.numeric(sqrt(1/x["a"] + 1/x["b"] + 1/x["c"] + 1/x["d"]))

Z <- qnorm(1 - 0.05/2)

ciLn0R <- c(log(orHat) - Z * seLn0R, log(orHat) + Z seLn0R)

ci0R <- exp(ciLn0R)
ci0R <- c(orHat, ci0R)

abort.tab <- matrix(c(222, 225, 201, 227, 113, 223, 18, 12), ncol = 4, byrow = TRUE)
dimnames(abort.tab) <- 
...
abort.tab <- matrix(c(222, 225, (222 + 225), 201, 277, (201 + 277), 113, 223, (113 + 223), 18, 12, (18 + 12)), nrow=4, byrow=TRUE)
...
oddsratio(convict.tab, method="wald")$measure[-2]
oddsratio(convict.tab, method="wald")$measure[-1]
oddsratio(convict.tab, method="wald")
